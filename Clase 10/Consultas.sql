...
-- A) Listar los libros en inglés cuya cantidad de páginas sea superior al promedio de páginas
Select Avg(Paginas*1.0) From Libros

Select L.Titulo, I.Nombre, L.Paginas From Libros as L
Inner Join Idiomas I ON L.IDIdioma = I.ID
Where I.Nombre = 'Inglés' And L.Paginas > (
    Select Avg(Paginas*1.0) From Libros
)

-- B) Listar los libros cuya cantidad de págians sea superior al promedio de páginas de libros en inglés
Select L.Titulo, L.Paginas From Libros as L
Where L.Paginas > (
    Select Avg(Paginas*1.0) From Libros as L
...
-- Para cada Idioma la cantidad de libros registrados de las editoriales Debolsillo y Plaza y Janés
...
(
    ...
    Where F.Medio = 'D' And LxB.IDLibro = L.ID
) As CantDigital,
(
    Select Count(*) From Libros_x_Biblioteca as LxB
    Inner Join Formatos as F ON F.ID = LxB.IDFormato
    Where F.Medio = 'F' And LxB.IDLibro = L.ID
) As CantFisico
From Libros as L

-- En la siguiente consulta se resuelve algo similar con la cláusula UNION pero que no resuelve lo que solicita la consigna.
Select L.Titulo, 'Digital' as Medio, Count(*) As Cantidad From Libros as L
Inner Join Libros_x_Biblioteca as LxB ON L.ID = LxB.IDLibro
Inner Join Formatos as F ON F.ID = LxB.IDFormato
Where F.Medio = 'D'
Group By L.Titulo
Union
Select L.Titulo, 'Fisico' as Medio, Count(*) As Cantidad From Libros as L
Inner Join Libros_x_Biblioteca as LxB ON L.ID = LxB.IDLibro
Inner Join Formatos as F ON F.ID = LxB.IDFormato
Where F.Medio = 'F'
Group By L.Titulo
...
Where MiTabla.CantDigital > MiTabla.CantFisico
...
Where MiTabla.CantDigital = MiTabla.CantFisico And MiTabla.CantDigital > 0
